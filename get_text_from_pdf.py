...
def get_text(file):
    ...
    name = search(fr'(?i)(?<={"Фамилия, имя, отчество"} )(\w+\s*\w+\s*\w+)', result).group()
    city = search(fr'(?i)(?<={"город"} )(\w+)', result).group()
    street = search(fr'(?i)(?<={"улица"} )(\w+)', result).group()
    house = search(fr'(?i)(?<={"дом"} )(\w+)', result).group()
    flat = search(fr'(?i)(?<={"квартира"} )(\w+)', result).group()
    placeofwork = search(fr'(?i)(?<={"Место учебы, работы"} )(\w+\s*\w+)', result).group()
